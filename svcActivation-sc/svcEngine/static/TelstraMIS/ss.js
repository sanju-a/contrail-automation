	  ...
	  var handler = function() {
		if (xmlhttp.readyState == XML_READY_STATE_COMPLETED) {
		  ...
		  try {			
			...
			//suggested = { "query":"a", "results": [ {"name":"account plans for galaxy tab 10.1 and this is another", "type":"suggest"}, {"name":"account usage data for home phone", "type":"suggest"}, {"name":"account", "type":"suggest"}, {"name":"account message", "type":"suggest"}, {"name":"another account message", "type":"suggest"}] };
		  } catch (e) {
			...
			try {
			  switch (ss_protocol) {
				...
				default:
				  ...
				  if (suggestions && suggestions.length > 0 && finalCount < 5) {
					var found = false;
					ss_cached[qVal].g = [];
...
function ss_handleAllKey(e) {
  ...
  switch (kid) {
    ...
    case 16:  // "shift-tab".
      ss_qbackup = null;
      ss_dismissed = true;
      if (ss_form_element == "header-search-telstra") {
      	var qry = document.getElementById(ss_form_element).inpSearch.value;
      } else {
...
function ss_handleKey(e) {
  ...
  if (ss_form_element == "header-search-telstra") {
  	var qnow = (!ss_qbackup) ? fo.inpSearch.value : ss_qbackup;
  } else {
  ...
  switch (kid) {
    case 40:  // "key down".
      ...
      if (ss_processed(qnow)) {
        ...
        if (sum > 0) {
          ...
          if (suggestion.q) {
          	if (ss_form_element == "header-search-telstra") {
          		fo.inpSearch.value = suggestion.q;
          	} else {
          		fo.q.value = suggestion.q;
          	}            
          } else {
          	if (ss_form_element == "header-search-telstra") {
          		fo.inpSearch.value = ss_qbackup;
          	} else {
    ...
    default:
      ss_dismissed = false;
      if (ss_form_element == "header-search-telstra") {
      	if (fo.inpSearch.value == ss_qshown) {
        	...
        	ss_waiting++;
        	if (ss_allow_debug && ss_debug && ss_debug.getDebugMode()) {
          	ss_debug.addWaitDebugLine(fo.inpSearch.value, 'queue', ss_wait_millisec);
        	}
        	ss_key_handling_queue = setTimeout(
            	'ss_handleQuery("' + ss_escape(fo.inpSearch.value) + '", ' +
...
function ss_sf() {
  if (ss_form_element == "header-search-telstra") {
  	document.getElementById(ss_form_element).inpSearch.focus();
  } else {  
...
function ss_show(qry) {
  if (ss_form_element == "header-search-telstra") {
	var currentQry = document.getElementById(ss_form_element).inpSearch.value;
  } else {
  ...
  if (cnt > 0) {
    ...
    clsTxt.onclick = function() {
      ...
      ss_clear();  // This will always turn off ss_dismiss after bring search
                   // box into focus.
      if (ss_form_element == "header-search-telstra") {
      	var query = document.getElementById(ss_form_element).inpSearch.value;
      } else {
  ...
  if (g && g.length > 0) {
    ...
    for (var i = 0; i < g.length; i++) {
      ...
      if (g[i].q) {
        ...
        if (ss_form_element == "header-search-telstra") {
        	//alert(txtNode + "-" + txtNode.length);
        	if (txtNode.length > 32) {
				alt.innerHTML = txtNode.substr(0,32)+'...';
			} else {
		...
		//alt.innerHTML = txtNode;
			...
			if (navigator.userAgent.indexOf("Mac OS X") != -1) {
				if (txtNode.length > 57) {
					alt.innerHTML = txtNode.substr(0,50)+'...';
				} else {
					alt.innerHTML = txtNode;
				}			
			} else {
				if (txtNode.length > 57) {
					alt.innerHTML = txtNode.substr(0,53)+'...';
				} else {
					alt.innerHTML = txtNode;
				}
			}
