	...
	if (!result.error) {
		...
		if (entries != null && entries.length > 0 ) {
			if (!isProductPage) {
				...
				display.appendChild(getBlogHeading());
			}
			...
			for (var i = 0; i < entries.length; i++) {
				...
				var img="<img width='60' class='newimgclass' src='"+thumbnailImg+"'>";
	...
	if(val.indexOf("<img")>0 || val.indexOf("<IMG")>0){
		...
		if(thumbnailImg.indexOf(".jpg")>0)
			...
			thumbnailImg = thumbnailImg.substring(thumbnailImg.indexOf("src=")+5,thumbnailImg.indexOf(".png")+4);
                else if(thumbnailImg.indexOf(".gif")>0)
                        thumbnailImg = thumbnailImg.substring(thumbnailImg.indexOf("src=")+5,thumbnailImg.indexOf(".gif")+4);
		else thumbnailImg="";
	}	
...
function getDateAuthor(pubDtStr, author, feedurl, lastEntry, pageID) {
	...
	content = "<span class='post-date'>"+pubDtStr+ "</span> <br/><span class='author-link'><a href='"+authorURL+"'  onclick=trackAtlasBlog('"+pageID+"')>"+author+"</a></span>";
	else
